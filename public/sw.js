// Bmbtech Service Worker - Offline First PWA
...
const STATIC_CACHE = `Bmbtech-static-${CACHE_VERSION}`;
const DYNAMIC_CACHE = `Bmbtech-dynamic-${CACHE_VERSION}`;
const API_CACHE = `Bmbtech-api-${CACHE_VERSION}`;
...
const API_CACHE_TIME_HEADER = 'x-Bmbtech-sw-cached-at';
        ...
        return Promise.all(
          cacheNames
            .filter((name) => {
              ...
              // Delete old Bmbtech caches
              return name.startsWith('Bmbtech-') && 
                     name !== STATIC_CACHE && 
                     name !== DYNAMIC_CACHE &&
                     name !== API_CACHE;
            })
...
self.addEventListener('message', (event) => {
  ...
  if (event.data === 'clearCache') {
    caches.keys().then((names) => {
      names.forEach((name) => {
        if (name.startsWith('Bmbtech-') || name.startsWith('xtfetch-')) {
          caches.delete(name);
        }
      });
    });
...
self.addEventListener('push', (event) => {
  ...
  const options = {
    ...
    tag: data.tag || 'Bmbtech-notification',
    data: data.data || { url: '/' },
    vibrate: [100, 50, 100],
